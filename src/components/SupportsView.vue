			...
			<div class="commentContent main">
				...
				<div class="comment">
					<div class="balloon">
						...
						<span class="owner">{{ support.user.name }} - {{ support.dt_update }}</span>
						<span class="text">{{ support.description }}</span>
					</div>
				</div>
				<button class="btn primary">
					<span v-if="showSupport === support.id" @click.prevent="showSupport = '0'"
						>Ocultar respostas</span
					>
					<span v-else @click.prevent="showSupport = support.id"
						>Exibir respostas (total: {{ support.replies.length }})</span
					>
				</button>
			</div>
			<div class="answersContent" v-show="showSupport === support.id">
				<div
					:class="['commentContent', 'rightContent' ?? support.user.id != reply.user.id]"
				...
				>
					<span class="avatar" v-if="support.user.id == reply.user.id">
						<img
							:src="[
								reply.user.image ? reply.user.image : require('@/assets/images/avatars/user03.svg'),
							]"
							:alt="reply.user.name"
						/>
					</span>
...
import ModalSupport from './ModalSupportView.vue';
export default {
	name: 'SupportsView',
	setup() {
		...
		return { supports, showSupport, modal, openModal };
	},
	components: {
		ModalSupport,
	},
